...
function MUILabel:init(id, label_data)
	...
	local inftex, infrec = managers.experience:rank_icon_data(self.rank)
...
function MUILabel.load_options(force_load)
	...
	MUILabel._muiStop = data.mui_label_stop ~= false;
...
function MUILabel:criminal()
	if not self._criminal and self.unit then
		self._criminal = managers.criminals:character_by_unit(self.unit);
	end
...
function MUILabel:resize()
	...
	local xxv = self.rank > 0 and self._muiInfamy;
